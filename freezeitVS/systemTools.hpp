...
public:
    ...
    SystemTools(Freezeit &freezeit, Settings &settings, ManagedApp &managedApp) :
            freezeit(freezeit), managedApp(managedApp), settings(settings) {
    ...
    void checkBattery() {
        ...
        if (lastCapacity == 0) { // 开机时
            ...
            const int charge_full_design = Utils::readInt(
    ...
    void bindCluster() {
        ...
        cpuCluster = res.size() == 0 ? 0 : ([res]() -> int {
            int tmp{0};
