...
private:
    ...
    const set<string> whiteListDefault{
...
public:
    ...
    void applyCfgTemp() {
        auto isSystemApp = [](const char *ptr) {
            const char *prefix[] = {
                    "com.miui.",
                    "com.oplus.",
                    "com.coloros.",
    ...
    void saveConfig() {
        auto fd = open(cfgPath.c_str(), O_WRONLY | O_TRUNC | O_CREAT, 0666);
        if (fd < 0) {
            freezeit.log("更新配置文件失败: [%s]", cfgPath.c_str());
            return;
        }
