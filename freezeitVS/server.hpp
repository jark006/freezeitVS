...
private:
    ...
    enum cmdEnum {
        ...
        printProcWCHAN = 62, // return string: "log" //打印冻结状态并返回log
...
public:
    ...
    Server(Freezeit &freezeit, Settings &settings, ManagedApp &managedApp, SystemTools &systemTools,
           Freezer &freezer) :
            ...
            systemTools(systemTools), freezer(freezer) {
        serverThread = thread(&Server::serverThreadFunc, this);
    }

    void serverThreadFunc() {
        constexpr int RECV_BUF_SIZE = 2 * 1024 * 1024;  // 2 MiB TCP通信接收缓存大小
        constexpr int REPLY_BUF_SIZE = 8 * 1024 * 1024; // 8 MiB TCP通信回应缓存大小
        ...
        auto recvBuf = make_unique<char[]>(RECV_BUF_SIZE);
        auto replyBuf = make_unique<char[]>(REPLY_BUF_SIZE);
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (replyLen) {
                    *reinterpret_cast<uint32_t *>(dataHeader) = replyLen;
                    send(clnt_sock, dataHeader, sizeof(dataHeader), MSG_DONTROUTE);
