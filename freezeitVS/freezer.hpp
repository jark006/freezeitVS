...
*/
    ...
    void printWchan() {
        ...
        size_t len = 0;
        char wchanStr[1024 * 8];

        STRNCAT(wchanStr, len, "进程冻结状态:\n\n"
                               " PID | MiB |  状 态  | 进 程\n");
        ...
        while ((file = readdir(dir)) != nullptr) {
            ...
            if (curForegroundApp.contains(uid)) {
                STRNCAT(wchanStr, len, "%5d %4d 📱正在前台 %s\n", pid, memMiB, label.c_str());
                continue;
            }

            if (pendingHandleList.contains(uid)) {
                STRNCAT(wchanStr, len, "%5d %4d ⏳等待冻结 %s\n", pid, memMiB, label.c_str());
                continue;
            }
            ...
            STRNCAT(wchanStr, len, "%5d %4d ", pid, memMiB);
            if (!strcmp(readBuff, v2wchan)) {
                STRNCAT(wchanStr, len, "❄️V2冻结中 %s\n", label.c_str());
            } else if (!strcmp(readBuff, v1wchan)) {
                STRNCAT(wchanStr, len, "❄️V1冻结中 %s\n", label.c_str());
            } else if (!strcmp(readBuff, SIGSTOPwchan)) {
                STRNCAT(wchanStr, len, "🧊ST冻结中 %s\n", label.c_str());
            } else if (!strcmp(readBuff, v2xwchan)) {
                STRNCAT(wchanStr, len, "❄️V2*冻结中 %s\n", label.c_str());
                fakerV2Cnt++;
        ...
        if (uidSet.size() == 0) {
            ...
            if (needRefrezze) {
                STRNCAT(wchanStr, len, "\n ⚠️ 发现 [未冻结] 的进程, 即将进行冻结 ⚠️\n");
                refreezeSecRemain = 0;
            }

            STRNCAT(wchanStr, len, "\n总计 %d 应用 %d 进程, 占用内存 ", (int) uidSet.size(),
                    (int) pidSet.size());
            STRNCAT(wchanStr, len, "%.2f GiB", totalMiB / 1024.0);
            if (fakerV2Cnt)
                STRNCAT(wchanStr, len, ", 共 %d 进程处于不完整V2冻结状态", fakerV2Cnt);
            if (isV1Mode())
                STRNCAT(wchanStr, len, ", V1已冻结状态可能会识别为[运行中]，请到[CPU使用时长]页面查看是否跳动");

            freezeit.log(wchanStr);
        }
        END_TIME_COUNT;
    }
    ...
    void processPendingApp() {
        ...
        for (; it != pendingHandleList.end();) {
            ...
            const int delta =
                    info.startRunningTime != 0 ? (time(nullptr) - info.startRunningTime) : 0;
    ...
    [[noreturn]] void cycleThreadFunc() {
        ...
        while (true) {
            ...
            if (remainTimesToRefreshTopApp > 0) {
                ...
                if (doze.isScreenOffStandby) {
                    if (doze.checkIfNeedToExit()) {
                        ...
                        updateAppProcess();
                        systemTools.setWakeupLockByLocalSocket(WAKEUP_LOCK::DEFAULT);
                    }
                } else {
#ifdef __x86_64__
            ...
            if (doze.checkIfNeedToEnter()) {
                ...
                updateAppProcess();
                systemTools.setWakeupLockByLocalSocket(WAKEUP_LOCK::IGNORE);
            }
